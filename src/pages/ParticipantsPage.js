...
function applySortFilter(array, comparator, query) {
  ...
  if (query) {
    const filteredValues = filter(
      array,
      (_participant) =>
        _participant["lastName"].toLowerCase().indexOf(query.toLowerCase()) !==
          -1 ||
        _participant["firstName"].toLowerCase().indexOf(query.toLowerCase()) !==
          -1
    );
...
function ParticipantsPage() {
  ...
  useEffect(() => {
    const filteredParticipants = applySortFilter(
      participants,
